...
export class CacheInterceptor implements HttpInterceptor{
    ...
    //pass along non-cachable requests and invalidate cache
        if(req.method !== 'GET') {
        ...
        if(cachedResponse){
            console.log(`Returning a cached tresponse: ${cachedResponse.url}`);
            console.log(cachedResponse);
            return of(cachedResponse);
        }

        //send requests to server and add response to cache
        return next.handle(req)
        .pipe(
            tap(event => {
